...
namespace SurvivalGameplayTags
{
	...
	UE_DEFINE_GAMEPLAY_TAG_COMMENT(InputTag_SecondInteract, "InputTag.SecondInteract", "Second interact input.");
	...
	UE_DEFINE_GAMEPLAY_TAG_COMMENT(Movement_Mode_Custom, "Movement.Mode.Custom", "Custom Character movement tag");
	...
	FGameplayTag FindTagByString(const FString& TagString, bool bMatchPartialString)
	{
		...
		if (!Tag.IsValid() && bMatchPartialString)
		{
			...
			for (const FGameplayTag& TestTag : AllTags)
			{
				...
				{
					LOGF_FUNC(Display, "Could not find exact match for tag [%s] but found partial match on tag [%s].", *TagString, *TestTag.ToString())
					Tag = TestTag;
					break;
				}
			}
