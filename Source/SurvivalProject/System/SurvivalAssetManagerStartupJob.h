...
DECLARE_DELEGATE_OneParam(FSurvivalAssetManagerStartupJobSubstepProgress, float /*NewProgress*/)

// Handles reporting progress from streamable handles
struct FSurvivalAssetManagerStartupJob
{
	FSurvivalAssetManagerStartupJobSubstepProgress SubstepProgressDelegate;
	TFunction<void(const FSurvivalAssetManagerStartupJob&, TSharedPtr<FStreamableHandle>&)> JobFunc;
	FString JobName;
	float JobWeight;
	mutable double LastUpdate = 0;

	// Simple job that is all synchronous
	FSurvivalAssetManagerStartupJob(const FString& InJobName,
	                                const TFunction<void(const FSurvivalAssetManagerStartupJob&,
